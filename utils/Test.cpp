#include <iostream>
#include <cassert>
#include "../Estimators/CardinalityEstimator.h"

#define IS_TRUE(x) \
    if (!(x)) { \
        std::cerr << __FUNCTION__ << " failed on line " << __LINE__ << ": " << #x << std::endl; \
        assert(x); \
    }
...
public:
    ...
    static void TestFindFirstSetBit() {
        uint32_t tail =  0b00000000100000000000000000000001; // Bit 24 is set
        ...
        IS_TRUE(FindFirstSetBit(ExtractLowBits(value,5), 6) == 1); // 0x0000001
    }
